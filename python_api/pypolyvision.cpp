...
VisibleAreaPythonWrapper::VisibleAreaPythonWrapper(
    const py::array_t<double> &origin, const py::list &fieldsOfView,
...
py::array_t<double> VisibleAreaPythonWrapper::getVisibilityBorder(
    const py::array_t<double> &line_xys) const {
...
// member functions of BeamDataGenerator
BeamDataGeneratorWrapper::BeamDataGeneratorWrapper(
    const py::array_t<double> &world_bound_poly, const py::list &obstacles,
    const py::array_t<double> &beam_directions) {
  Polygon_2 world_bounds = numpyArray2Polygon_2(world_bound_poly);
  Polygon_set_2 obs_ps = listOfNumpyArrays2Polygon_set_2(obstacles);
  std::vector<Vector_2> beam_dirs = numpyArray2VecOfVector_2(beam_directions);
  std::vector<double> beam_angl;

  this->beamDataGenerator = std::make_unique<BeamDataGenerator>(
      world_bounds, obs_ps, beam_dirs, beam_angl);
}

py::array_t<double>
BeamDataGeneratorWrapper::getSensorbeamIntersectPointsAtPose(
    const py::array_t<double> &p, const double &theta) const {
  Point_2 pos;
  pos = numpyArray2Point_2(p);
  std::vector<Point_2> beam_intersects =
      this->beamDataGenerator->getSensorbeamIntersectPointsAtPose(pos, theta);

  return point_2Vec2NumpyArray(beam_intersects);
}

bool BeamDataGeneratorWrapper::isPointInObstacles(
    const py::array_t<double> &p) const {
  Point_2 pos;
  pos = numpyArray2Point_2(p);

  return this->beamDataGenerator->isPointInObstacles(pos);
}

bool BeamDataGeneratorWrapper::isPointInWorld(const py::array_t<double>& p) const {
  Point_2 pos;
  pos = numpyArray2Point_2(p);
  return this->beamDataGenerator->isPointInWorld(pos);
}

py::list BeamDataGeneratorWrapper::getObstacles() const {
  return polygon_set_2_2ListOfNumpyArrays(this->beamDataGenerator->obstacles_);
}

py::array_t<double> BeamDataGeneratorWrapper::getWorldBounds() const {
  return polygon_2_2NumpyArray(this->beamDataGenerator->world_bounds_);
}
...
py::array_t<double> polygon_2_2NumpyArray(const Polygon_2 &polygon) {
  ...
  polygonArray.resize(
      {static_cast<ssize_t>(polygon.size()), static_cast<ssize_t>(2)});
  auto rawPolygonArray =
      polygonArray.mutable_unchecked<2>(); // get mutable raw pointer to data
  // also possible:
...
Polygon_2 numpyArray2Polygon_2(const py::array_t<double> &polygonArray) {
  ...
  Polygon_2 polygon;
  // polygon.resize(numberOfPoints); // this creates a std::vector<IntPoint>
  // with numberOfPoints (0,0) IntPoints in it
  // --> do not
  // use p.push_back()! (since it will add points at the end of the vector, but
  // overwrite points with p[i])
...
py::list polygon_set_2_2ListOfNumpyArrays(const Polygon_set_2 &polygons) {
  ...
  std::list<Polygon_with_holes_2> polygonList =
      cgalUtils.convertPolygonset2PolygonList(polygons);
...
bool checkInsidePythonWrapper(const py::array_t<double> &point,
...
std::vector<Vector_2>
numpyArray2VecOfVector_2(const py::array_t<double> &vecs_array) {
  // check dimensions first
  if (vecs_array.ndim() != 2) {
    throw std::invalid_argument(
        (std::string("Number of dimension must be 2, but is ") +
         std::to_string(vecs_array.ndim()) + std::string(".\n"))
            .c_str());
  }
  // optional: add check for shape of pathArray to be (n,2)
  // OR: just take only first two columns (done here!)
  auto rawArray = vecs_array.unchecked<2>();
  ssize_t numberOfVecs = rawArray.shape(0);

  std::vector<Vector_2> vecs;
  vecs.reserve(numberOfVecs);

  for (ssize_t i = 0; i < numberOfVecs; i++) {
    vecs.emplace_back(Vector_2(rawArray(i, 0), rawArray(i, 1)));
  }
...
py::array_t<double> point_2Vec2NumpyArray(const std::vector<Point_2> &points) {
  py::array_t<double> point_arr;
  point_arr.resize(
      {static_cast<ssize_t>(points.size()), static_cast<ssize_t>(2)});
  auto rawPoint_arr = point_arr.mutable_unchecked<2>();
  int i = 0;
  for (const auto &p : points) {
    rawPoint_arr(i, 0) = CGAL::to_double(p.x());
    rawPoint_arr(i, 1) = CGAL::to_double(p.y());
    i++;
  }
  return point_arr;
}
...
PYBIND11_MODULE(PYTHON_API_MODULE_NAME, m) {
  ...
  m.def("checkInside", &checkInsidePythonWrapper,
        "A function which adds two numbers");

  py::class_<VisibleAreaPythonWrapper>(m, "VisibleArea", py::module_local())
      .def(py::init<const py::array_t<double> &, const py::list &,
                    const py::list &>(),
           py::arg("origin"), py::arg("fieldsOfView"),
      ...
      .def("getVisibleAreas", &VisibleAreaPythonWrapper::getVisibleAreas,
           "Returns the visible areas as list of numpy arrays.")

      .def("getNonVisibleAreas", &VisibleAreaPythonWrapper::getNonVisibleAreas,
           "Returns the non-visible areas as list of numpy arrays.")

      .def("checkInside", &VisibleAreaPythonWrapper::checkInside,
           "Returns the non-visible areas as list of numpy arrays.")

      .def("getOpaquePolygons", &VisibleAreaPythonWrapper::getOpaquePolygons,
           "Returns the opaque polygons (obstacles) as list of numpy arrays.")

      .def("getFieldsOfView", &VisibleAreaPythonWrapper::getFieldsOfView,
           "Returns the fields of view as list of numpy arrays.")

      .def("getVisibilityBorder",
           &VisibleAreaPythonWrapper::getVisibilityBorder,
           "Returns the border of visible area and given linestring a numpy "
           "array of shape (1,2).")

      .def("getOrigin", &VisibleAreaPythonWrapper::getOrigin,
           "Returns the origin as a numpy array of shape (1,2).");

  py::class_<BeamDataGeneratorWrapper>(m, "BeamDataGenerator",
                                       py::module_local())
      .def(py::init<const py::array_t<double> &, const py::list &,
                    const py::array_t<double> &>(),
           py::arg("world_bound_poly"), py::arg("obstacles"),
           py::arg("beam_directions"))
    
    .def("getSensorbeamIntersectPointsAtPose", &BeamDataGeneratorWrapper::getSensorbeamIntersectPointsAtPose)

    .def("isPointInObstacles", &BeamDataGeneratorWrapper::isPointInObstacles)

    .def("isPointInWorld", &BeamDataGeneratorWrapper::isPointInWorld)

    .def("getObstacles", &BeamDataGeneratorWrapper::getObstacles)

    .def("getWorldBounds", &BeamDataGeneratorWrapper::getWorldBounds);
